...
export function Inject(
   valueToProvide: any,
): <T>(classConstructor: { new (...args: any[]): T }, methodName: undefined, parameterIndex: number) => void;
...
export function Inject(valueToProvide: any, methodName?: undefined, parameterIndex?: number): any {
   if (parameterIndex === undefined) {
      ...
      if (!paramTypes) {
         throw new Error(
            `InjectionError: No type emitted for parameter ${parameterIndex + 1} of ${classConstructor.name}.`,
         );
      }
...
function getDiTokens(classConstructor: Function): any[] {
   const existing =
      Reflect.getMetadata('di-tokens', classConstructor) || Reflect.getMetadata('design:paramtypes', classConstructor);
