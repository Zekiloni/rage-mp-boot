...
function getDiTokens(classConstructor: Function): any[] {
   ...
   throw new InjectionError(
      'Class with dependencies is provided but not decorated with @Injectable or @Inject',
      classConstructor,
   );
}
...
function throwOnCircularDependencies(dependencies: any[], factories: Map<any, Factory>, dependents: any[]): void {
   for (const dependency of dependencies) {
      if (dependents.indexOf(dependency) >= 0) {
         throw new InjectionError(
            `Circular dependency: "${tokenName(dependency)}" is required by one of its dependencies.`,
            dependency,
         );
      }
